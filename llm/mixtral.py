...
class MixtralInfer:
    ...
    def load_models_tokenizer(self, model_path):
        ...
        model.generation_config.repetition_penalty = 0.95  # disable repetition penalty
    ...
    def preprocess_prompt(self, question, choices, context=None):
        user_message_ = USER_MESSAGE_MISTRAL.format(question=question, answer_choices=choices)
        if context is not None:
            user_message_ = USER_MESSAGE_MISTRAL_WITH_CONTEXT.format(context=context, question=question,
                                                                   answer_choices=choices)
        prompt = DEFAULT_PROMPT_MISTRAL.format(user_message=user_message_)
        return prompt
